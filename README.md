...
Titan `TradingVenue` adapter for the Quay program, packaged as a
standalone repo for the Titan team. The root crate is
`quay-aggregator-titan`; it depends on two vendored Quay crates so the
repo builds with no external Quay sources:

| Crate | Path | Purpose |
| --- | --- | --- |
| `quay-aggregator-titan` | `/` (root) | `TradingVenue` impl for routers using `titan-integration-template` |
...
`titan-integration-template` is `edition = "2024"`, so rustc >= 1.85 is
required.
...
Quay crates, trimmed to what the adapter needs (DSL authoring/template
